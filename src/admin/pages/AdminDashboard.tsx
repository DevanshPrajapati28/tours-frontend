  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 48, height: 48, border: '4px solid #e2e8f0', borderTopColor: '#0f4c81', borderRadius: '50%', margin: 'auto', animation: 'spin 1s linear infinite' }} />
  ...
  return (
    <div>
      ...
      {stats && stats.newInquiries > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '.875rem', padding: '1rem 1.25rem', borderRadius: 12, background: 'rgba(255,107,53,.08)', border: '1px solid rgba(255,107,53,.2)', marginBottom: '1.5rem' }}>
          <Clock size={20} style={{ color: '#ff6b35', flexShrink: 0 }} />
          <p style={{ fontSize: '.9rem', color: '#1a2332' }}>
            You have <strong style={{ color: '#ff6b35' }}>{stats.newInquiries} new inquiry{stats.newInquiries > 1 ? 'ies' : ''}</strong> waiting for your response.
          </p>
          <Link to="/admin/inquiries" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, fontSize: '.8125rem', fontWeight: 600, color: '#ff6b35', textDecoration: 'none', flexShrink: 0 }}>
      ...
      <div style={{ borderRadius: 16, background: '#fff', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Link to="/admin/inquiries" style={{ fontSize: '.8125rem', fontWeight: 600, color: '#0f4c81', textDecoration: 'none' }}>View all →</Link>
        </div>
      ...
      <div style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '1rem' }}>
        {[
          { label: 'Add New Package', to: '/admin/packages?action=new', icon: Package, color: '#0f4c81' },
          { label: 'View All Inquiries', to: '/admin/inquiries', icon: MessageSquare, color: '#ff6b35' },
