  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)', fontFamily: 'var(--font-sans)' }}>
      ...
      <aside style={{
        ...
        background: 'linear-gradient(180deg, #0f4c81 0%, #0a355c 100%)', color: '#fff', display: 'flex', flexDirection: 'column',
        ...
        <div style={{ padding: '1.25rem 1rem', borderBottom: '1px solid rgba(255,255,255,.1)', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'flex-start' : 'center', gap: '.75rem', overflow: 'hidden', height: '76px', transition: 'all .25s ease' }}>
          ...
          {sidebarOpen && (
            <div style={{ overflow: 'hidden', whiteSpace: 'nowrap' }}>
              ...
              <p style={{ fontSize: '.65rem', fontWeight: 700, letterSpacing: '.18em', textTransform: 'uppercase', color: '#ff6b35', marginTop: 4 }}>Admin</p>
            </div>
          )}
        </div>
        ...
        <button onClick={() => setSidebarOpen(v => !v)} style={{
          ...
          border: '1px solid #e2e8f0', color: '#0f4c81', cursor: 'pointer',
              ...
              {({ isActive }) => (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={20} style={{ flexShrink: 0, color: isActive ? '#ff6b35' : 'inherit' }} />
                  </div>
                  {sidebarOpen && (
      ...
      <main style={{ flex: 1, marginLeft: sidebarW, transition: 'margin-left .25s cubic-bezier(0.4, 0, 0.2, 1)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div style={{ width: 42, height: 42, borderRadius: 12, background: 'linear-gradient(135deg, #ff6b35 0%, #e85a28 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '1.125rem', boxShadow: '0 4px 12px rgba(255, 107, 53, 0.3)' }}>
